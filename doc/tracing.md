# User-space, Statically Defined Tracing (USDT) for TerraAustralis Core

TerraAustralis Core includes statically defined tracepoints to allow for more
...
**Note**: TerraAustralis Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to TerraAustralis Core
...
To list probes in TerraAustralis Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in TerraAustralis Core.
