# Support for Output Descriptors in TerraAustralis Core

Since TerraAustralis Core v0.17, there is support for Output Descriptors. This is a
...
on TerraAustralis's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a TerraAustralis Core descriptor wallet to serve as your
...
existing TerraAustralis Core wallets, a convenience function `combo` is
...
All RPCs in TerraAustralis Core will include the checksum in their output. Only
