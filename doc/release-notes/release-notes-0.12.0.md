TerraAustralis Core version 0.12.0 is now available from:
...
ECDSA signatures inside TerraAustralis transactions now use validation using
...
Previous versions of TerraAustralis Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of TerraAustralis Core was to raise the default minimum
relay fee.

TerraAustralis Core 0.12 will have a strict maximum size on the mempool. The
...
TerraAustralis Core 0.12 also introduces new default policy limits on the length and
...
TerraAustralis Core 0.12 nodes. TerraAustralis Core will only allow replacement of
...
Note that the wallet in TerraAustralis Core 0.12 does not yet have support for
...
TerraAustralis Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. TerraAustralis Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In TerraAustralis Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in TerraAustralis Core 0.13, and it is at this time undecided whether the
...
TerraAustralis Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
TerraAustralis Core automatically creates a hidden service to listen on, without
manual configuration. TerraAustralis Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if TerraAustralis Core is listening, and
...
TerraAustralisd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals TerraAustralis
Core to use floating fees. By default, TerraAustralis Core will use floating
fees.
...
At all times, TerraAustralis Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, TerraAustralis Core will never create transactions paying less than
...
- #6214 `87406aa` [OSX] revert renaming of TerraAustralis-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
