TerraAustralis Core version 0.18.0 is now available from:
...
`/Applications/TerraAustralis-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
recommended to use TerraAustralis Core on unsupported systems.
...
10.10. Additionally, TerraAustralis Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the TerraAustralis Core
...
`~bitcoin/.bitcoin`. This change makes TerraAustralis Core more consistent with
...
- A new document introduces TerraAustralis Core's BIP174 [Partially-Signed
  TerraAustralis Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent TerraAustralis-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  TerraAustralis Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when TerraAustralis Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of TerraAustralis Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside TerraAustralis
...
This section describes planned changes to TerraAustralis Core that may affect
other TerraAustralis software and services.

- Since version 0.16.0, TerraAustralis Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with TerraAustralis Core 0.20 (expected about
  a year after 0.18), TerraAustralis Core will default to native segwit
  ...
  to bech32 addresses, and if the TerraAustralis Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in TerraAustralis Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any TerraAustralis Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, TerraAustralis Core
...
- A sub-project of TerraAustralis Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with TerraAustralis Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to TerraAustralis Core's own implementation, although entropy
  gathered by TerraAustralis Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves TerraAustralis Core a
...
- On macOS, TerraAustralis Core now opts out of application CPU throttling
...
- #11625 Add TerraAustralisApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for TerraAustralis Core (sipsorcery)
...
- #12775 Integration of property based testing into TerraAustralis Core (Christewart)
