# CJDNS support in TerraAustralis Core

It is possible to run TerraAustralis Core over CJDNS, an encrypted IPv6 network that
...
done outside of TerraAustralis Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the TerraAustralis network and individual nodes.
...
before it will work with your TerraAustralis Core node. This is described in steps
...
## Run TerraAustralis Core with CJDNS

Once you are connected to the CJDNS network, the following TerraAustralis Core
...
When enabled, this option tells TerraAustralis Core that it is running in an
...
IPv6 local network. This helps TerraAustralis Core perform better address management:
...
CJDNS support was added to TerraAustralis Core in version 23.0 and there may be fewer
...
## CJDNS-related information in TerraAustralis Core

There are several ways to see your CJDNS address in TerraAustralis Core:
