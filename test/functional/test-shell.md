...
The `TestShell` submodule extends the `TerraAustralisTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `TerraAustralisTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `TerraAustralisTestFramework` between
...
## 2. Importing `TestShell` from the TerraAustralis Core repository

We can import the `TestShell` by adding the path of the TerraAustralis Core
...
The `TestShell` inherits all `TerraAustralisTestFramework` members and methods, such
as:
...
`TerraAustralisTestFramework` object. The full set of argument keywords which can be
...
Unlike the `TerraAustralisTestFramework` class, the `TestShell` keeps the underlying
TerraAustralisd subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `bitcoind` node
processes and `TerraAustralisTestFramework` convenience methods can be accessed
interactively.
...
underlying `TerraAustralisTestFramework`:
...
`TerraAustralisTestFramework` class. The user can override these in
