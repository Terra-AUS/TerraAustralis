#!/bin/bash
# Copyright (c) 2013 The TerraAustralis Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete TerraAustralis database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no TerraAustralis datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old TerraAustralis datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected TerraAustralis 0.7 datadir."
    ;;
  3)
    echo "Detected TerraAustralis pre-0.8 datadir."
    ;;
  4)
    echo "Detected TerraAustralis 0.8 datadir."
    ;;
esac
